...
for label, path in files.items():
    ...
    # Normalize frequencies as percentages
...
plt.title("MMST Distribution Based on HSV Mapping for predicted_images(with lighting)", fontsize=14, fontweight="bold")
...
plt.xlabel("Monk Skin Tone (MST) Scale", fontsize=12, fontweight="bold")
plt.ylabel("Percentage of Dataset Representing Each Frequency (%)", fontsize=12, fontweight="bold")
plt.title("MST Distribution Based on YCbCr Mapping for predicted_images(with lighting))", fontsize=14, fontweight="bold")
plt.xticks(x_positions + bar_width * 1.5, range(1, 11))
plt.legend(loc="upper right")
plt.grid(axis="y", linestyle="--", alpha=0.6)
plt.tight_layout()
