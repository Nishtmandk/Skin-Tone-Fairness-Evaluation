...
class TRUST(object):
    ...
    def test(self, return_params=False):
        ...
        elif self.config.test_data == "ffhq_val":
            testdata = datasets_test.FFHQFaceDataset(image_size=self.config.image_size, scene_size=self.config.scene_size, scale=[self.config.scale_min, self.config.scale_max], trans_scale=self.config.trans_scale)
        else:
        ...
        for iter, batch_data in enumerate(test_loader):
            ...
            for col_num in range(self.config.batch_size):
                ...
                visdict_albedo = {
                    'inputs': images[visind], #input
                    'predicted_images': predicted_images_alpha[visind], #FLAME 3d face model with light
                    'albedo_images': predicted_albedo_images[visind], # FLAME 3d face model without light
                    'albedo': albedo[visind], # TRUST etimated albedo map
                    
                }
                savepath = os.path.sep.join([self.config.savefolder, image_name + '.png'])
                if not os.path.exists(os.path.dirname(savepath)):
                    os.makedirs(os.path.dirname(savepath))
                self.visualize(visdict_albedo, savepath)
                # Create folders for each output type
                output_folders = {
                "inputs": os.path.join(self.config.savefolder, "inputs"),
                "predicted_images": os.path.join(self.config.savefolder, "predicted_images"),
                "albedo_images": os.path.join(self.config.savefolder, "albedo_images"),
                "albedo": os.path.join(self.config.savefolder, "albedo"),
                }

                 # Automatically create all required folders
                for folder_name, folder_path in output_folders.items():
                 os.makedirs(folder_path, exist_ok=True)

                 # Save images in the test loop
                for col_num in range(self.config.batch_size):
                   visind = np.arange(col_num, col_num + 1)
    
                 # Define save paths
                input_savepath = os.path.join(output_folders["inputs"], f"{images_names[col_num]}.jpg")
                predicted_savepath = os.path.join(output_folders["predicted_images"], f"{images_names[col_num]}.png")
                albedo_image_savepath = os.path.join(output_folders["albedo_images"], f"{images_names[col_num]}.png")
                albedo_savepath = os.path.join(output_folders["albedo"], f"{images_names[col_num]}.png")
    
                 # Save images
                torchvision.utils.save_image(images[visind], input_savepath)  
                torchvision.utils.save_image(predicted_images_alpha[visind], predicted_savepath)  
                torchvision.utils.save_image(predicted_albedo_images[visind], albedo_image_savepath)  
                torchvision.utils.save_image(albedo[visind], albedo_savepath)  
